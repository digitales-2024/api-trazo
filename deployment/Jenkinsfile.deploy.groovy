            ...
            steps {
                ...
                script {
                    withDockerRegistry(credentialsId: "${REGISTRY_CREDENTIALS}") {
                        // build docker image
                        def image = docker.build("${IMAGE_PREFIX}-api:${BUILD_NUMBER}-test")
                        image.push()
                    }
                }
        ...
        stage("Restart backend service") {
            steps {
                sshagent(['ssh-deploy']) {
                    ...
                    sh "${SSH_COM} 'cd ${REMOTE_FOLDER} && sed -i -E \"s/image: ${ESCAPED_IMAGE_PREFIX}-api:.+\$/image: ${ESCAPED_IMAGE_PREFIX}-api:${BUILD_NUMBER}-test/\" docker-compose.yml'"
